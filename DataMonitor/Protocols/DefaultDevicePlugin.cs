...
/// <summary>
/// 通用传感器设备插件 — 使用 DefaultProtocol (0xAA 0x55, XOR校验)
/// 端口8888
/// </summary>
public class DefaultDevicePlugin : IDevicePlugin
{
    private readonly DefaultProtocol _proto = new();
    ...
    public string PluginName => "通用传感器";
    ...
    public async Task ConnectAsync(string ip, int port, CancellationToken ct = default)
    {
        ...
        _cts = new CancellationTokenSource(); _tcp = new TcpClient();
        await _tcp.ConnectAsync(ip, port, ct); _ns = _tcp.GetStream(); _cnt = 0;
        Log("已连接"); Conn(true);
    ...
    {
        _cts?.Cancel(); _tcp?.Close(); _ns?.Dispose();
        _tcp = null; _ns = null; _cts?.Dispose(); _cts = null; _cnt = 0;
        Log("已断开"); Conn(false); await Task.CompletedTask;
    }
    ...
    public async Task<float> ReadParameterAsync(ParameterId pid, CancellationToken ct = default)
    {
        ...
        catch (OperationCanceledException) { throw new TimeoutException($"读参超时:{pid}"); }
        finally { _pending = null; }
        throw new InvalidOperationException($"读参失败:{pid}");
    }
    ...
    public async Task<bool> WriteParameterAsync(ParameterId pid, float v, CancellationToken ct = default)
    {
        ...
        catch (OperationCanceledException) { throw new TimeoutException($"写参超时:{pid}"); }
        finally { _pending = null; }
        return false;
    }
    ...
    private void Log(string m) => LogMessage?.Invoke(this, $"[{PluginName}] {m}");
