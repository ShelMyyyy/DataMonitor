...
/// <summary>
/// 温度监测仪插件 — 使用 TemperatureProtocol (0xBB 0x66, 求和校验)
/// 端口8889
/// </summary>
public class TemperatureMonitorPlugin : IDevicePlugin
{
    private readonly TemperatureProtocol _proto = new();
    ...
    public async Task<float> ReadParameterAsync(ParameterId pid, CancellationToken ct = default)
    {
        ...
        catch (OperationCanceledException) { throw new TimeoutException($"读参超时"); }
    ...
    private void Log(string m) => LogMessage?.Invoke(this, $"[温度] {m}");
